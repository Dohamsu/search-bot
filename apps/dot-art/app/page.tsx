...
import DotArtInfoSection from "./components/DotArtInfoSection";
...
export default function Home() {
  ...
  const handleImageConvert = useCallback((convertedGrid: DotGrid) => {
    ...
    setHistory(addHistoryItem(historyRef.current, convertedGrid, "auto", t("sections.photoToDotArt")));
    setToast({ message: t("toast.photoConverted"), type: "success" });
  }, [t]);
  ...
  const handleProGenerate = useCallback(
    (result: { grid: DotGrid; imageDataUrl: string }) => {
      ...
      setHistory(addHistoryItem(historyRef.current, result.grid, "pro", "AI"));
      setToast({ message: t("toast.aiGenerated"), type: "success" });
    },
    [t]
  );
  ...
  const handleHistoryLoad = useCallback((item: HistoryItem) => {
    setGrid(item.grid);
    setRawProImageUrl(null);
    setToast({ message: t("toast.historyLoaded"), type: "success" });
  }, [t]);
      ...
      <header className="border-b border-gray-100 bg-white/80 backdrop-blur-sm">
        <div className="mx-auto max-w-6xl px-4 py-4 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-4 text-sm text-gray-500">
            <a href="/privacy" className="hover:text-gray-700 transition-colors">{t("header.privacy")}</a>
            <a href="/terms" className="hover:text-gray-700 transition-colors">{t("header.terms")}</a>
            <LanguageSwitcher />
          </nav>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
        <div className="flex gap-6">
          ...
          <div className="flex-1 min-w-0 space-y-6">
            ...
            {/* Customization */}
            <div className="rounded-2xl bg-white p-6 shadow-sm border border-gray-100">
              <h2 className="text-base font-semibold text-gray-900 mb-4">{t("sections.customizing")}</h2>
              <DotArtCustomizer
          ...
          <div className="hidden lg:block lg:sticky lg:top-8 lg:self-start w-[340px] shrink-0">
            <div className="rounded-2xl bg-white p-6 shadow-sm border border-gray-100">
              <h2 className="text-base font-semibold text-gray-900 mb-4">{t("sections.preview")}</h2>
              <DotArtPreview
                grid={grid}
                options={{
        ...
        <DotArtInfoSection />
        <RelatedTools currentToolId="dot" />
